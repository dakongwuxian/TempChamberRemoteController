...
class Chamber:
    ...
    def get_run_status(self):
        resp = self.send_cmd("rREAD RUN STATUSr")
        if "OK" in resp:
    ...
    def get_temp(self):
        resp = self.send_cmd("rREAD TEMPr")
        if "OK" in resp:
    ...
    def set_temp(self, t):
        resp = self.send_cmd(f"rSET TEMP {t}r")
        return "OK" in resp
...
class ChamberGUI(tk.Tk):
    def __init__(self):
        ...
        # IP选择
        ttk.Label(ctrl_frame, text="Select Chamber:").grid(row=0, column=0, sticky="e")
        ...
        self.conn_btn.grid(row=0, column=2, padx=10)

        # 运行状态
        ttk.Label(ctrl_frame, text="Status：").grid(row=1, column=0, sticky="e")
        self.status_var = tk.StringVar(value="disconnected")
        ttk.Label(ctrl_frame, textvariable=self.status_var).grid(row=1, column=1, sticky="w")

        # 当前温度
        ttk.Label(ctrl_frame, text="Temperture：").grid(row=2, column=0, sticky="e")
        self.temp_var = tk.StringVar(value="--")
        ttk.Label(ctrl_frame, textvariable=self.temp_var).grid(row=2, column=1, sticky="w")

        # 目标温度设置
        ttk.Label(ctrl_frame, text="Target temp：").grid(row=3, column=0, sticky="e")
        self.set_temp_var = tk.StringVar()
        ttk.Entry(ctrl_frame, textvariable=self.set_temp_var, width=10).grid(row=3, column=1, sticky="w")
        ttk.Button(ctrl_frame, text="Set", command=self.on_set_temp).grid(row=3, column=2, padx=5)
...
# 此段空白用以分开面板和函数 ---------------------------------------------------------------------------------------------------------------
    ...
    def mark_new_point_without_draw(self):
        ...
        if not all_points:  # 如果没有点，直接返回
            return
        # 最后一个点的 (x, y) 坐标
        x_last, y_last = all_points[-1]  # 最后一个点的 (x, y) 坐标
        ...
        # 添加文本标注
        label_text = f'{x_last.strftime("%H:%M:%S")} \n{y_last:.1f}℃'
        # 计算标注位置的偏移量
        # 获取当前坐标轴的范围
        x_min, x_max = self.ax.get_xlim()
        y_min, y_max = self.ax.get_ylim()
        ...
        # 添加文本标注，位置在点的右上方
        text = self.ax.text(
            mdates.date2num(x_last) + x_offset, 
            y_last + y_offset,
            label_text,
        ...
        """
        在 data_scatter 的最后一个点处添加一次标记（黄色圆环 + 文本），
        并移除上次 mark 时产生的所有标记。
        不执行 draw()，调用者可统一再 draw_idle()。
        """
    ...
    def _poll_loop(self):
        ...
        # 更新 UI
        self.after(0, lambda s=status_code, t=temp_value: self._update_ui(s, t))
    ...
    def _update_ui(self, status_code, temp_value):
        # 更新运行状态
        text = {0:"stop",1:"run",2:"pause"}.get(status_code, "error")
